...
from Encryption.JPEG.jacdecColorHuffman import jacdecColor
from Encryption.JPEG.jdcdecColorHuffman import jdcdecColor
from Encryption.JPEG.Quantization import *
from Encryption.JPEG.invzigzag import invzigzag
from Encryption.JPEG.zigzag import zigzag
from Encryption.encryption_utils import loadEncBit, loadImgSizes
import tqdm

# 64 histograms for each cipherimage
DC_bin_interval = [i for i in range(-1170, 1170, 36)]
AC_bin_interval = [i for i in range(-1170, 1170, 36)]
...
def extract_feature(dc, ac, size, type, QF, N=8):
    ...
    for j in range(0, allblock8.shape[2]):
        temp = allblock8[:, :, j]
        allcoe[j, :] = zigzag(temp)#把每个block展开，依次得到64个系数
    #alloce是block_num x 64的二维矩阵
    hist_img = np.zeros(AC_histgram_dimension * 63 + DC_histgram_dimension)
    for j in range(0, 64):#取第j列，即代表第j个系数，其中第0个就是直流系数
        if j == 0:
...
def extract_all_component_feature(QF=90):
    ...
    for k in tqdm.tqdm(range(image_num)):
        ...
        dcallY, acallY, dcallCb, acallCb, dcallCr, acallCr = loadEncBit('../data/PlainJPEGbit_1K', k)
    ...
    data.to_csv(feature_save_path + '/' + 'PlainDCTHistfeats_1K.csv'.format(QF), index=False, header=0)
